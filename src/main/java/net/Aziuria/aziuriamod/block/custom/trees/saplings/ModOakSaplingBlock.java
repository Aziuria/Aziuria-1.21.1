...
public class ModOakSaplingBlock extends SaplingBlock {

    // 70% modded, 30% vanilla
    private static final int MODDED_CHANCE = 6; // out of 10

    public ModOakSaplingBlock(Properties properties) {
    ...
    private void growRandomTreeVariant(ServerLevel level, BlockPos pos, BlockState state, RandomSource random) {
        ...
        if (random.nextInt(10) < MODDED_CHANCE) {
            ...
            ModTreeGrowers.OAK_VARIANT_1.growTree(level, generator, pos, state, random);
        } else {
            // VANILLA OAK
            TreeGrower.OAK.growTree(level, generator, pos, state, random);
        }
    }
}